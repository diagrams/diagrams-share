...
-- Slight modification the the function encodeGifAnimation and writeGigAnimation
-- so that we can turn off dithering.

encodeGifAnim :: GifDelay -> GifLooping -> Bool
                   -> [Image PixelRGB8] -> Either String (L.ByteString)
encodeGifAnim delay looping dithering lst =
    encodeGifImages looping
        [(pal, delay, img)
                | (img, pal) <- palettize
                    defaultPaletteOptions {enableImageDithering=dithering} <$> lst]

writeGifAnim :: FilePath -> GifDelay -> GifLooping -> Bool
                  -> [Image PixelRGB8] -> Either String (IO ())
writeGifAnim path delay looping dithering img =
    L.writeFile path <$> encodeGifAnim delay looping dithering img
...
main = do
  ...
  case args of
    (inFilename : numImages : delay : outFile : _) -> do
      ...
      -- The False below turns off dithering, make it True to enable it
      let eitherResut = writeGifAnim outFile (read delay) LoopingForever False rgb8s
      case eitherResut of
        Left s -> putStrLn s
        Right io -> io
